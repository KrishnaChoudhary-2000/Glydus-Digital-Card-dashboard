...
const CardPreview: React.FC<{ data: ExecutiveData | null; onUpdate: (updates: Partial<ExecutiveData>) => void; isDraggable?: boolean; }> = ({ data, onUpdate, isDraggable = true }) => {
  ...
  const handleLogoMouseMove = useCallback((e: MouseEvent) => {
    if (!dragInfo.current.isDragging || !headerRef.current || !data) return;
    dragInfo.current.hasDragged = true;
    const headerRect = headerRef.current.getBoundingClientRect();
    
    // Calculate new raw pixel position of top-left corner
    const x = e.clientX - headerRect.left - dragInfo.current.offsetX;
    const y = e.clientY - headerRect.top - dragInfo.current.offsetY;

    // The logo is centered on its position, so calculate the center's new position
    const logoElement = headerRef.current.querySelector('a');
    const logoWidth = logoElement?.offsetWidth || data.companyLogoSize;
    const logoHeight = logoElement?.offsetHeight || 40;

    let newXPercent = ((x + logoWidth / 2) / headerRect.width) * 100;
    let newYPercent = ((y + logoHeight / 2) / headerRect.height) * 100;
    
    // Clamp values to keep the logo's center within the box
  ...
  const handleLogoMouseDown = (e: React.MouseEvent) => {
    if (!isDraggable || !headerRef.current) return;
  ...
  useEffect(() => {
    // Cleanup listeners if the component unmounts during a drag
    return () => {
      document.removeEventListener('mousemove', handleLogoMouseMove);
      document.removeEventListener('mouseup', handleLogoMouseUp);
    };
  }, [handleLogoMouseMove, handleLogoMouseUp]);
    ...
    <div className="relative bg-brand-card bg-gradient-to-b from-brand-card to-black rounded-3xl shadow-2xl w-full max-w-sm mx-auto transition-all duration-500 font-sans overflow-hidden" style={buttonStyle}>
      ...
      >
        <a
          ...
          onClick={(e) => { if (dragInfo.current.hasDragged) e.preventDefault(); }}
          className="absolute cursor-move transition-transform duration-300 hover:scale-105"
          style={{
        ...
        >
          {companyLogoUrl ? (
            <img src={companyLogoUrl} alt={`${data.companyName} logo`} draggable={false} className="max-h-[80px] w-full object-contain pointer-events-none" />
          ) : (
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center font-sans" onClick={onClose}>
      <div className="bg-brand-card border border-white/10 rounded-2xl shadow-2xl p-8 w-full max-w-lg m-4 animate-fade-in-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-blue-900/50 border border-blue-700 text-blue-200 text-sm rounded-lg p-4 mb-6">
          <p><strong className="font-semibold">Share this public link!</strong> Anyone with this link can view the digital card. To ensure the link is a manageable length and works reliably everywhere, images are not included in the shared version.</p>
        </div>
...
const PublicCardPage: React.FC<{ data: ExecutiveData }> = ({ data }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 bg-brand-dark font-sans">
      <div className="relative perspective-container">
        ...
        >
          <div className="card-face card-front">
            <CardPreview data={data} onUpdate={() => {}} isDraggable={false} />
          </div>
          <CardBack data={data} />
        </div>
      </div>
          ...
          <FormSection title="Profile">
            ...
            <div className="flex items-center gap-4">
             ...
             <div className="pt-4 space-y-4 border-t border-white/10 mt-4">
                ...
                <button 
                    onClick={() => handleCardDataChange({ companyLogoPosition: { x: 50, y: 50 } })}
                    className="w-full text-sm bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-3 rounded-lg transition disabled:opacity-50"
                    disabled={editorDisabled}
                >
                    Reset Logo Position
                </button>
